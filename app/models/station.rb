class Station
  ...
  def ns
    @ns ||= all.map {|station| station[:label] if station[:code].start_with? 'NS'.freeze}.compact
  end

  def ew
    @we ||= all.map {|station| station[:label] if station[:code].start_with? 'EW'.freeze}.compact
  end

  def cg
    @cg ||= all.map {|station| station[:label] if station[:code].start_with? 'CG'.freeze}.compact
  end

  def ne
    @ne ||= all.map {|station| station[:label] if station[:code].start_with? 'NE'.freeze}.compact
  end

  def cc
    @cc ||= all.map {|station| station[:label] if station[:code].start_with? 'CC'.freeze}.compact
  end

  def dt
    @dt ||= all.map {|station| station[:label] if station[:code].start_with? 'DT'.freeze}.compact
  end

  def te
    @te ||= all.map {|station| station[:label] if station[:code].start_with? 'TE'.freeze}.compact
  end

  def ce
    @ce ||= all.map {|station| station[:label] if station[:code].start_with? 'CE'.freeze}.compact
  end

  def interchanges
    return @interchanges if @interchanges.present?

    station_names = (ns + ew + cg + ne + cc + dt + te + ce)
    @interchanges = station_names.select { |name| station_names.count(name) > 1 }.uniq
  ...
  def code_by_name(name)
    ...
    return station[:code] if station
    nil
  end

  def code_by_label(label)
